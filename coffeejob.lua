...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(10)
	if PlayerData.job ~= nil and PlayerData.job.name == 'hesukeano' and not isBlip then 
	isBlip = true
...
AddEventHandler('brewing:animation', function()
	TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BBQ", 0, true) -- Brewing Animation. You can change the animation to something else to be more realistic.
	Citizen.Wait(10000) -- Time progress , in ms, 1 second = 1000 ms
	ClearPedTasksImmediately(PlayerPedId()) -- Function to stop the animation
end)
...
function openMenu()
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'cooking',
        {
            ...
            align    = 'top-right',
            elements = { 
                { label = 'Donut ', value = 'menu1' }, -- Change the "Menu 1" label to your combination item 
                { label = 'Cookie ', value = 'menu2' }
            }
        },
    function(data, menu)
